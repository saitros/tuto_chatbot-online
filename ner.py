...
class Parser:
    ...
    def load_models(self, loc=None):
        if loc == None:
            ...
            # loading word2Idx
            self.word2Idx = np.load("Entity/word2Idx.npy").item()
            # loading idx2Label
            self.idx2Label = np.load("Entity/idx2Label.npy")
            # loading char2Idx
            self.char2Idx = np.load("Entity/char2Idx.npy").item()
        else:
            self.model = load_model(os.path.join(loc,"model.h5"))
            # loading word2Idx
            self.word2Idx = np.load(os.path.join(loc, "Entity/word2Idx.npy")).item()
            # loading idx2Label
            self.idx2Label = np.load(os.path.join(loc, "Entity/idx2Label.npy")).item()
            # loading char2Idx
            self.char2Idx = np.load(os.path.join(loc, "Entity/char2Idx.npy")).item()
